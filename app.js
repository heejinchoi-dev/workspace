...
// 이 아래로 Util 섹션 함수들이 이어지게 두시면 됩니다.
// 모바일 사이드바 토글
  ...
  ['기획중','개발중','QA/테스트','배포완료','보류'].forEach(function(s){
    ...
    new Sortable(colEl, {
      ...
      onEnd: function(evt) {
        ...
        var projectId = evt.item.getAttribute('onclick').match(/'([^']+)'/)[1];
...
function openDevModal(data){
  ...
  // openDevModal 내 마감일 입력 부분에 아래 코드를 추가하세요
'<div class="grid grid-cols-2 gap-4 mb-4">'+
  '<div><label class="block text-xs font-bold text-gray-500 mb-2">시작일</label><input id="dev-start" type="date" class="w-full border p-4 r24"></div>'+
  '<div>'+
    '<label class="flex justify-between text-xs font-bold text-gray-500 mb-2"><span>마감일</span>'+
    '<label class="flex items-center gap-1 cursor-pointer"><input type="checkbox" id="no-deadline" onchange="document.getElementById(\'dev-end\').disabled=this.checked"> 없음</label></label>'+
    '<input id="dev-end" type="date" class="w-full border p-4 r24"></div>'+
'</div>'
}
